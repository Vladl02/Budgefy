...
export default function EditBudgetModal({ visible, onClose, initialItems, onSave }: Props) {
  ...
  useEffect(() => {
    if (visible) {
      ...
      const mappedItems = initialItems.map(item => ({
        ...item,
        color: item.color || CATEGORY_STYLES[item.icon].color
      }));
      setLocalItems(JSON.parse(JSON.stringify(mappedItems)));
      resetForm();
    }
  }, [visible, initialItems, panY]);
  ...
  const startEditing = (item: SummaryItem) => {
    ...
    setSelectedColor(item.color || CATEGORY_STYLES[item.icon].color);
  };
  ...
  const handleSubmit = () => {
    ...
    const isDuplicate = localItems.some(item => {
        ...
        return item.icon === selectedIconKey && (item.color || CATEGORY_STYLES[item.icon].color) === selectedColor;
    });
  ...
  const renderItem = ({ item }: { item: SummaryItem }) => {
    ...
    const activeColor = item.color || categoryStyle.color;
